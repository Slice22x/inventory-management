...
const StatCard = ({ title, primaryIcon, details, dateRange }: Props) => {
  ...
  return (
    <div className="md:row-span-1 xl:row-span-2 bg-white col-span-1 shadow-md rounded-2xl flex flex-col justify-between">
      {/* HEADER */}
      <div>
        <div className="flex justify-between items-center mb-2 px-5 py-4">
            ...
            <React.Fragment key={index}>
              <div className="flex items-center justify-between my-4">
                ...
                <div className="flex items-center">
                  <detail.IconComponenet
                    className={`w-4 h-5 mr-1 ${getChangeColour(
                      detail.changePercentage
                    )}`}
                  />

                  <span
                    className={`font-medium ${getChangeColour(
                      detail.changePercentage
                    )}`}
                  >
